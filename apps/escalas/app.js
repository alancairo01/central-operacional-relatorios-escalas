...
const DEFAULT_STATE = {
  ...
  employees: [
    ...
    { nome: "Edinho Maia", cargo: "Eletricista" },
  ],
  shifts: {
    "Controlador de Operações": [
...
const COVERAGE_ROLES_FOR_TECH_SHIFT = [
  "Téc.sistema Audio visual",
  "Supervisor Técnico",
  "Controlador de Operações",
];
...
function applySundayCountSequence(name, startWeekIndex, startValue) {
  ...
  for (let weekIndex = startWeekIndex; weekIndex < weeks.length; weekIndex++) {
    const key = `${name}|${weekIndex}`;
    state.manualSundayCounts[key] = Math.max(0, Math.min(currentValue, 3));
    currentValue = Math.min(currentValue + 1, 3);
  }
}
  ...
  for (const day of state.schedule) {
    ...
    for (const role of ROLES) {
      ...
      for (const shift of shifts) {
        ...
        // Aos domingos, o turno 00:00–06:00 dos técnicos não é obrigatório.
        ...
        /*
          Regra de cobertura:
          Para os horários de Técnico de Sistemas Audio Visuais, a cobertura é considerada válida
          quando houver alguém no mesmo horário entre:
          - Técnico de Sistemas Audio Visuais;
          - Supervisor Técnico;
          - Controlador de Operações.

          Assim, se não houver técnico naquele horário, mas houver Supervisor ou Controlador,
          o horário conta como coberto.
        */
        const coverageRoles = role === TECH_ROLE ? COVERAGE_ROLES_FOR_TECH_SHIFT : [role];

        const hasCoverage = state.employees
          .filter(emp => emp.nome && coverageRoles.includes(emp.cargo))
          .some(emp => {
            const value = getManual(emp.nome, day.date) || day.assignments[emp.nome] || "";
            return shiftCoversRequiredTime(value, shift.inicio, shift.fim);
          });

        if (!hasCoverage) {
          const roleText = role === TECH_ROLE
            ? "Téc.sistema Audio visual / Supervisor / Controlador"
            : role;

          violations.push({ date: day.date, role: roleText, shift: label });
        }
      }
    }
  }
...
function shiftCoversRequiredTime(value, requiredStart, requiredEnd) {
  const parsed = parseCellTime(value);
  if (!parsed || isBlock(value)) return false;

  const workedStart = timeToMinutes(parsed.start);
  let workedEnd = timeToMinutes(parsed.end);
  const reqStart = timeToMinutes(requiredStart);
  let reqEnd = timeToMinutes(requiredEnd);

  if (workedEnd <= workedStart) workedEnd += 24 * 60;
  if (reqEnd <= reqStart) reqEnd += 24 * 60;

  return workedStart <= reqStart && workedEnd >= reqEnd;
}
